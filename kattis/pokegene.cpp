...
void solve() {
    int N,Q;
    cin>>N>>Q;
    vector<pair<string,int>> pokemon(N);
    for(int i=0;i<N;i++){
        cin>>pokemon[i].first;
        pokemon[i].second=i+1;
    }
    sort(pokemon.begin(),pokemon.end());
    vector<int> pos_in_sorted(N+1);
    vector<string> strings(N);
    for(int i=0;i<N;i++){
        pos_in_sorted[pokemon[i].second]=i;
        strings[i]=pokemon[i].first;
    }
    LCP lcp(strings);
    while(Q--){
        int K,L;
        ...
        for(int i=0;i<K;i++){
            int idx;
            cin>>idx;
            xs.push_back(pos_in_sorted[idx]);
        }
        sort(xs.begin(),xs.end());
        int ans=0;
        for(int i=0;i<=K-L;i++){
            int L_shared=lcp.get_lcp(xs[i],xs[i+L-1]);
            int L_prev=(i>0)?lcp.get_lcp(xs[i-1],xs[i]):0;
            int L_next=(i+L<K)?lcp.get_lcp(xs[i+L-1],xs[i+L]):0;
            ans+=max(0,L_shared-max(L_prev,L_next));
        }
        cout<<ans<<"\n";
    }
    return;
