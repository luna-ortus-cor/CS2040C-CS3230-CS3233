// taken from https://www.comp.nus.edu.sg/~stevenha/cs2040c/demos/BSTDemo.cpp
  ...
  BSTVertex* remove(BSTVertex* T, T1 v) {
    ...
    if (T->key == v) {                           // the node to be deleted
      if ((T->left == NULL) && (T->right == NULL)) { // this is a leaf
        delete T; // this is correct, the proper removal
...
public:
  ...
  void insert(T1 v) { root = insert(root, v); }

  void inorder() { inorder(root); cout << '\n'; }

  void preorder() { preorder(root); cout << '\n'; }

  T1 findMin() { return findMin(root); }

  T1 findMax() { return findMax(root); }
