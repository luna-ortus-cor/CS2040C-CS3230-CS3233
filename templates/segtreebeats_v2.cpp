...
/**
 * Segment Tree Beats
 * Supports: Range Add, Range Chmin, Range Chmax, Range Sum, Range Mod, Range (Integer) Div
...
private:
    ...
    void range_mod(int p, int L, int R, int i, int j, ll m) {
        if (R < i || L > j || tree[p].max1 < m) return;
    
        if (L == R) {
            ll v = tree[p].max1 % m;
            tree[p] = {v, v, -INF, 1, v, INF, 1, 0};
            return;
        }
        ...
        int mid = (L + R) >> 1;
        range_mod(p << 1, L, mid, i, j, m);
        range_mod(p << 1 | 1, mid + 1, R, i, j, m);
    
        push_up(p);
    }

    void range_div(int p, int L, int R, int i, int j, ll k) {
        if (R < i || L > j) return;
    
        // all become 0
        if (tree[p].max1 < k) {
            // equivalent to set to 0
            tree[p] = {0, 0, -INF, R-L+1, 0, INF, R-L+1, 0};
            return;
        }
    
        // uniform segment
        if (tree[p].max1 == tree[p].min1) {
            ll v = tree[p].max1 / k;
            tree[p] = {v * (R-L+1), v, -INF, R-L+1, v, INF, R-L+1, 0};
            return;
        }
        ...
        int mid = (L + R) >> 1;
        range_div(p << 1, L, mid, i, j, k);
        range_div(p << 1 | 1, mid + 1, R, i, j, k);
...
public:
    ...
    void mod(int l, int r, ll m) { range_mod(1, 0, n - 1, l, r, m); }
    void divide(int l, int r, ll k) { range_div(1, 0, n - 1, l, r, k); }
};
